#!/usr/bin/env python3
"""
PFE VALIDATION - Fusion Caméra + LiDAR
Sortie: 3 parties comme dans le schéma
"""
...
class PFEValidation(Node):
    def __init__(self):
        super().__init__('pfe_validation')
        ...
        self.pub_validation = self.create_publisher(Image, '/pfe/validation', 10)
        
        self.get_logger().info("PFE VALIDATION - Pret")
    ...
    def lidar_callback(self, msg):
        ...
        cv2.putText(img_camera, "Camera data", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        # 2. IMAGE LIDAR (nuage de points)
        ...
        for p in points:
            ...
            if pixel:
                u, v = pixel
                cv2.circle(img_lidar, (u, v), 1, (255, 255, 255), -1)
        cv2.putText(img_lidar, "LiDAR data", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(img_lidar, f"Points: {len(points)}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
        
        # 3. FUSION + DEPTH ESTIMATION
        ...
        cv2.putText(img_fusion, "Data fusion and Depth estimation", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        
        # Légende profondeur
        cv2.putText(img_fusion, "Proche (<2m): Rouge", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
        cv2.putText(img_fusion, "Moyen (2-5m): Orange", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 165, 255), 1)
        cv2.putText(img_fusion, "Loin (5-10m): Jaune", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
        cv2.putText(img_fusion, "Tres loin (>10m): Bleu", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
        
        # ASSEMBLAGE (3 parties côte à côte)
        h, w = self.image_height, self.image_width
        canvas = np.zeros((h, w * 3, 3), dtype=np.uint8)
        canvas[0:h, 0:w] = img_camera
        canvas[0:h, w:w*2] = img_lidar
        canvas[0:h, w*2:w*3] = img_fusion
        ...
        # Titre général
        cv2.putText(canvas, "Camera and LiDAR data", (w//2 - 100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        cv2.putText(canvas, "Data fusion and Depth estimation", (w*2 + w//2 - 130, h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        
        # FPS
        cv2.putText(canvas, f"FPS: {self.fps:.1f}", (10, h - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
        
        img_msg = self.bridge.cv2_to_imgmsg(canvas, 'bgr8')
        self.pub_validation.publish(img_msg)
